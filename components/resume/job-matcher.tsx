...
export function JobMatcher({ resume }: JobMatcherProps) {
  ...
  async function startMatching() {
    ...
    try {
      ...
      setMatchResult(data.match as JobMatchResult);
    } catch (err: unknown) {
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <p className="text-sm text-muted-foreground">
            Paste the job description you&apos;re interested in, and our AI will analyze how
            well your current resume matches the requirements.
          </p>
          <textarea
            ...
            rows={8}
            className="w-full rounded-xl border border-border/60 bg-background/50 p-4 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 resize-none"
          />
              ...
              <CardContent>
                <div className="flex flex-wrap gap-2">
                  {matchResult.missingSkills.length > 0 ? (
                    ...
                    <p className="text-sm text-muted-foreground">
                      You have all the critical skills listed.
                    </p>
                  )}
                </div>
              </CardContent>
