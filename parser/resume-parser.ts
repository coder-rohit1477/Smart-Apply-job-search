...
export async function getResumeProfile(userId?: string): Promise<ResumeProfile> {
  ...
  const skills = latestResume.parsedData.skills.slice(0, 3).map((skill) => skill.name);
  const strengths = skills.length > 0 ? skills : defaultResumeProfile.strengths;
  ...
  const seniority: ResumeProfile["seniority"] =
    yearsOfExperience >= 8 ? "Lead" : yearsOfExperience >= 4 ? "Senior" : "Mid-level";
...
export function getKeywordOverlap(profile: ResumeProfile, skills: string[]): string[] {
  const normalizedProfileKeywords = new Set(
    profile.keywords.map((keyword) => keyword.toLowerCase()),
  );

  return skills.filter((skill) =>
    normalizedProfileKeywords.has(skill.toLowerCase()),
  );
}
